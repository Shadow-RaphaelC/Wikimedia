...
public class MediasController : Controller
{
            ...
            if (DB.Medias.HasChanged || forceRefresh)
            {
                ...
                else
                {
                    ...
                    result = DB.Medias.ToList()
                        .Where(m => m.Shared || m.OwnerId == currentUserId.ToString());
                }
    ...
    {
        // B.7
        Media.OwnerId = Models.User.ConnectedUser.Id.ToString();
        DB.Medias.Add(Media);
        return RedirectToAction("List");
    }
    ...
    public ActionResult Edit()
    {
        ...
        if (id != 0)
        {
            ...
            {
                // B.7
                User current = Models.User.ConnectedUser;
                if (Media.OwnerId != current.Id.ToString() && !current.IsAdmin)
                    return RedirectToAction("List");
    ...
    public ActionResult Edit(Media Media)
    {
        ...
        if (storedMedia != null)
        {
            // B.7
            User current = Models.User.ConnectedUser;
            if (storedMedia.OwnerId != current.Id.ToString() && !current.IsAdmin)
                return RedirectToAction("List");
